...
import type { CellItem, CellModelAsset, ViewMode } from "../data/schema/cell";
...
function disposeMeshMaterials(material: Mesh["material"]) {
...
function disposeClonedScene(root: Object3D) {
  root.traverse((node) => {
...
function AssetCellModel({
  ...
  const clonedScene = useMemo(() => {
    ...
    clone.traverse((node) => {
      ...
      disposeMeshMaterials(previousMaterial);
      meshIndex += 1;
    });

    return clone;
  }, [cell, scene, viewMode, crossSection, asset.materialMode, asset.exposure]);
...
function EcoliModel({ activeOrganelle, viewMode, crossSection }: CommonModelProps) {
  const flagellumPaths: Array<Array<[number, number, number]>> = [
    [
      [-1.38, 0.06, 0.16],
      [-2.05, 0.42, 0.38],
      [-2.78, 0.14, 0.68],
  ...
  return (
    <group rotation={[0.08, -0.38, 0.06]} scale={[1.06, 1.06, 1.06]}>
      <mesh rotation={[0, 0, Math.PI / 2]} castShadow receiveShadow>
        <capsuleGeometry args={[0.52, 2.35, 14, 44]} />
        <CellMaterial
          id="cellWall"
          activeOrganelle={activeOrganelle}
          viewMode={viewMode}
          color="#c2185b"
          opacity={crossSection ? 0.34 : 0.68}
        />
      </mesh>
      <mesh rotation={[0, 0, Math.PI / 2]} scale={[0.9, 0.9, 0.84]}>
        <capsuleGeometry args={[0.42, 2.12, 12, 36]} />
        <CellMaterial
          id="cellEnvelope"
          activeOrganelle={activeOrganelle}
          viewMode={viewMode}
      ...
      <CurveTube
        ...
        viewMode={viewMode}
      />
      {flagellumPaths.map((points, index) => (
        <CurveTube
          key={`flagellum-${index}`}
          id="flagellum"
          color="#f8bbd9"
    ...
    <group ref={group} position={[0, 0, 0]}>
      {cell.modelAsset ? (
        ...
        <>
          ...
          {cell.modelKind === "ecoli" && <EcoliModel {...common} />}
